...
public class AuthorServiceTests
{
    ...
    public async Task GetAllAuthorsAsync_ShouldReturnAuthors_FromCache_WhenCacheExists()
    {
        ...
        // Act
        var result = await _service.getAllAuthorAsync(new CancellationToken());
        ...
        _authorRepoMock.Verify(r => r.getAllAuthorAsync(new CancellationToken()), Times.Never);
    }
    ...
    public async Task GetAllAuthorsAsync_CacheEmpty_FetchesFromRepositoryAndSetsCache()
    {
        ...
        _authorRepoMock.Setup(r => r.getAllAuthorAsync(new CancellationToken()))
                       .ReturnsAsync(authorsFromRepo);

        var service = new AuthorService(_authorRepoMock.Object, _mapper, _cacheMock.Object);

        // Act
        var result = await service.getAllAuthorAsync(new CancellationToken());
        ...
        _cacheMock.Verify(c => c.SetAsync("Authors", It.IsAny<ICollection<AuthorReadDto>>()), Times.Once);
    }
}